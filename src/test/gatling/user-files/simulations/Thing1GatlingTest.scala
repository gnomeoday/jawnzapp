...
/**
 * Performance test for the Thing1 entity.
 */
class Thing1GatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Thing1 entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all thing1s")
            .get("/services/jawnzapp/api/thing-1-s")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new thing1")
            .post("/services/jawnzapp/api/thing-1-s")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_thing1_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created thing1")
                .get("/services/jawnzapp${new_thing1_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created thing1")
            .delete("/services/jawnzapp${new_thing1_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
