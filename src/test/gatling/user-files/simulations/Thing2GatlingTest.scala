...
/**
 * Performance test for the Thing2 entity.
 */
class Thing2GatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Thing2 entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all thing2s")
            .get("/services/jawnzapp/api/thing-2-s")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new thing2")
            .post("/services/jawnzapp/api/thing-2-s")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_thing2_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created thing2")
                .get("/services/jawnzapp${new_thing2_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created thing2")
            .delete("/services/jawnzapp${new_thing2_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
